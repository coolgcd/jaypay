...
class WalletHelper
{
    public static function getMemberEarnings($memberId)
    {
        ...
        $incomeHistory = DB::table('member_income_history')
                    ->where('is_laps', 0)
            ->where('member_id', $memberId)->sum('amount');
    ...
    public static function checkAllMembersCapping($memberId)
    {
        ...
        foreach ($members as $member) {
            ...
            $capAmount = $isWorking ? $payment * 3 : $payment * 2;

            $earnings = self::getMemberEarnings($memberId);
            $totalEarned = $earnings['total_income'] ?? 0;

            if ($totalEarned >= $capAmount) {
                // Wrap in transaction if needed for consistency
    ...
    public static function checkSingleMemberCapping($memberId)
{
    ...
    $totalIncome = $member->tot_income_amt ?? 0;
    $totalCap = $member->tot_cpping_amt ?? 0;
...
public static function storeLapsAmountLog($memberId, $lapsAmount, $reason = 'Cap limit reached',$matchingPairs = NULL)
{
    try {
        ...
        DB::table('member_laps_log')->insert([
            ...
            'amount'    => $lapsAmount,
            'reason'    => $reason,
            'matching_pair' => $matchingPairs, // new field, optional
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    } catch (\Exception $e) {
