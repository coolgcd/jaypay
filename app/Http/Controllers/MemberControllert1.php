...
// {
//     $member = Auth::guard('member')->user(); // get the logged-in user
//     return view('member.dashboard', compact('member')); // pass to Blade
// }
// public function dashboard()
// {
//     $member = Auth::guard('member')->user(); // get the logged-in user
    
//     // Get network statistics using the member's show_mem_id
//     $networkStats = $this->getNetworkStats($member->show_mem_id);
    
//     return view('member.dashboard', compact('member', 'networkStats')); // pass both to Blade
// }
...
// $member = DB::table('member_binary')->where('memid', $member->show_mem_id)->first();

//     $ltot = '';

//     if (!empty($member->left)) {
//         $leftParent = $member->left;
//         WalletHelper::getLeftMembersRecursive($leftParent, $ltot);
//     }
...
//parth_dashboard_count_dont use this 
//  $member = DB::table('member_binary')->where('memid', $member->show_mem_id)->first();
    ...
    // if (!empty($member->left)) {
    //     $leftParent = $member->left;
    //     placementHelper::getLeftMembersRecursive($leftParent, $ltot);
    //     }
    ...
    //     $rightParent = $member->right;
    //     placementHelper::getRightMembersRecursive($rightParent, $rtot);
    //     }
...
//        if (!empty($member->show_mem_id)) {
//         $rootParent =$member->show_mem_id;
//         placementHelper::getAllMembersRecursive($rootParent, $atot);
//         }
//         // $atot = $rootParent.$atot;

//     // Clean up leading comma
//     $atot = ltrim($atot, ',');

//     //echo $atot;
//     echo count(explode(",", $atot));
...
//     $matchingIncome = DB::table('matching_income')
//         ->where('member_id', $member->show_mem_id)
//         ->sum('amount') ?? 0;
...
//     // UPDATE THIS: Add the new variables to compact
//     return view('member.dashboard', compact(
//         'member', 
//         'networkStats',
...
private function getNetworkStats($memId)
{
    ...
    // Dynamic total network count
    // $allCount = 0;
    // $atot = '';
    // placementHelper::getAllMembersRecursive($memId, $atot);
    // if (!empty($atot)) {
    //     $allCount = count(array_filter(explode(',', $atot)));
    // }
...
//     $stats = [
//         'my_network' => $this->getTotalNetworkSize($memId),
//         'direct_members' => $this->getDirectMembers($memId),
//         'active_direct' => $this->getActiveDirectMembers($memId),
//         'left_network' => $currentMember->tot_left ?? 0,
//         'right_network' => $currentMember->tot_right ?? 0,
//         'active_left' => $this->getActiveLeftMembers($memId),
//         'active_right' => $this->getActiveRightMembers($memId)
//     ];
...
// public function showProfile()
// {
//     $member = Auth::guard('member')->user()->load('bankDetails');
//     return view('member.profile', compact('member'));
// }
...
// private function buildTree($memid)
// {
//     $memberBinary = MemberBinary::with(['member', 'leftChild.member', 'rightChild.member'])
//         ->where('memid', $memid)
//         ->first();
...
//         'name' => $memberBinary->member->name,
//         'status' => $memberBinary->member->status == 1,
//         'left' => $memberBinary->left 
//             ? $this->buildTree($memberBinary->left)
//             : null,
//         'right' => $memberBinary->right 
//             ? $this->buildTree($memberBinary->right)
//             : null,
//         'left_business' => number_format($payout->totleft_amount ?? 0, 2),
//         'right_business' => number_format($payout->totright_amount ?? 0, 2),
//     ];
...
//     // Get network stats using your own accurate logic
//     $stats = $this->getNetworkStats($memid);

//     // Get latest binary payouts for business data
//     $payout = DB::table('binary_payouts')
//         ->where('member_id', $memid)
//         ->latest('created_at')
//         ->first();

//     $joinDate = $binary->joindate ? date('d-m-Y', $binary->joindate) : 'N/A';
//     $activationDate = $binary->activatedate ? date('d-m-Y', $binary->activatedate) : 'N/A';

//     $response = [
//         'name' => $member->name,
//         'memid' => $member->show_mem_id,
//         'status' => $member->status == 1 ? 'Active' : 'Inactive',
//         'sponsor' => $binary->sponsor 
//             ? $binary->sponsor->name . ' (' . $binary->sponsor->show_mem_id . ')'
//             : 'N/A',
//         'upline' => $binary->upline 
...
//         'inactive_network_left' => 0, // Still left as-is
//         'inactive_network_right' => 0,
...
public function getMemberDetails($memid)
{
    ...
    if (!$member) { // No need to check for binary, if member doesn't exist, nothing will work.
        return response()->json(['error' => 'Member not found'], 404);
    }

    // --- CORRECTED LOGIC ---
    // Calculate real-time business volume from the placement tree.
    // This will work for ANY member, active or not.
    $leftBusiness = 0;
    $rightBusiness = 0;
    if ($binary) {
    ...
    $response = [
        ...
        'upline' => $binary && $binary->upline
            ...
            : 'N/A',
        'join_date' => $joinDate,
        'joining_packag' => $binary->payamount,
        'activation_date' => $activationDate,
        'active_direct' => $stats['active_direct'],
        'inactive_direct' => $stats['direct_members'] - $stats['active_direct'],
        'active_network_left' => $stats['active_left'],
        ...
        'total_team' => $stats['my_network'],
        'left_business' => number_format($leftBusiness, 2), // Use new calculated value
        'right_business' => number_format($rightBusiness, 2), // Use new calculated value
    ];
...
//             'emailid' => 'required|email|max:255|unique:member,emailid',
...
//         $member = new Member();
//         $member->show_mem_id = 'M' . time(); // Or use custom ID logic
//         $member->position = $request->position;
//         $member->name = $request->name;
//         $member->mobileno = $request->mobileno;
//         $member->emailid = $request->emailid;
//         $member->password = Hash::make($request->password);
//         $member->sponsorid = $sponsor->id;
//         $member->joindate = now();

//         // Set defaults for required non-null fields
//         $member->parentid = 0;
//         $member->tot_ref = 0;
//         $member->all_ref = 0;
//         $member->all_pool = 0;
...
//         $member->save();
//     return redirect()->route('member.register.success')->with('show_mem_id', $show_mem_id);
...
//     $generatedId = 'M' . time(); // Generate Member ID and save it for session
...
//     // Redirect with success and show_mem_id
//     return redirect()->route('member.register.success')->with('show_mem_id', $generatedId);
// }
...
// Alternative method if you want to show withdrawals in member details
// public function getMemberDetails($memid)
// {
//     // Get member details
//     $member = DB::table('member')->where('show_mem_id', $memid)->first();
...
//     // Get withdrawal records for this member
//     $withdrawals = DB::table('payment_withdraw')
//         ->where('member_id', $member->id)
//         ->orderBy('request_date', 'desc')
//         ->get();
    
//     return view('member.details', compact('member', 'withdrawals'));
// }
//start3
...
return view('member.direct_history', compact('memberId', 'incomeHistory', 'referredMember', 'totalReceived'));
    ...
    /**
     * Display member matching income with pagination
     */
    // public function memberMatchingIncome($memid)
    // {
    //     // Get member details
    //     $member = DB::table('member')->where('show_mem_id', $memid)->first();
    ...
    //     // Get matching income records with pagination
    //     $matchingIncome = DB::table('matching_income')
    //         ->where('member_id', $member->id)
    //         ->orderBy('id', 'desc')
    //         ->paginate(15);

    //     return view('member.matching_income', compact('member', 'matchingIncome', 'memid'));
    // }
public function memberMatchingIncome(Request $request)
{
    ...
    /**
     * Display member salary income with pagination
     */
 

    /**
     * Display member reward income with pagination
     */
